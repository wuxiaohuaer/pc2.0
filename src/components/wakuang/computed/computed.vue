...
export default {
  ...
  methods: {
        getdata1() {
            let url = this.http+'pc.php/Article/lists/p/1/cid/44/access_token/'+this.token
            fetch(url)
                .then(e => e.json())
                .then(e => {
...
<style  lang="less">
    .qukuailian{
        ...
        .des{
            background: yellow; 
            background: url(../../../assets/img/logo-12.png) no-repeat 26px 93px;
            .title{
                height: 83px;
                font: 24px/83px "黑体";
                text-indent: 24px;
